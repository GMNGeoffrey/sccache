...
Either download pre-built sccache binaries from a [release](https://github.com/mozilla/sccache/releases), or build sccache locally with the `dist-client` and `dist-server` features enabled:
```
